jQuery(document).ready(function($) {
    'use strict';
    var this_obj = vw_clothing_store_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.vw-clothing-store-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message vw-clothing-store-plugin-install');
                button.addClass('button-primary vw-clothing-store-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.vw-clothing-store-recommended-plugins .vw-clothing-store-activation-note').css('display','block');
    ...
    $(document).on('click', '.vw-clothing-store-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .vw-clothing-store-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .vw-clothing-store-recommended-plugins').css('display','none');
    });
